...
8. The Git Automation Exclusivity Law, the Approval-Gated Merge Automation Law, and the Mandatory Patch Version Law forbid manual git commands: every add/commit/push/merge must run through `scripts/git-sync.sh`, `scripts/run-safe.sh`, or `scripts/git-merge-with-approval.sh`, the commit/merge message must cite the active TODO, the approval artifact must match the HEAD SHAs/timestamp, and `VERSION.md` must include the new patch metadata before the Process Integrity and Batch Verification laws mark the run complete.
...
- TODO-072 formalizes the Audit Service track so it remains always-on: the roadmap/queue call the Audit Service a parallel governance stream, the Process Integrity Auditor keeps the audit metadata current, and the Mandatory Patch Version Law plus the Audit Integrity Law guarantee the audit snapshot, version bump, and merge approvals stay in sync before the template evolves again.

## CI as Mandatory Verifier
- The CI Verification Gate Law declares that GitHub Actions must run the canonical verification stack (`scripts/verify-roadmap-sync.sh`, `scripts/verify-version-sync.sh`, `scripts/process-integrity-sweep.sh`, `scripts/run-audit.sh`, `scripts/git-merge-report.sh`) on every `push` and `pull_request`, preserve the audit/branch/tree artifacts, and fail merges until the gates complete successfully from a clean checkout.
- Local automation remains the only execution path: humans still approve merges, while `scripts/run-safe.sh`, `scripts/git-sync.sh`, and `scripts/git-merge-with-approval.sh` write git history. CI mirrors those scripts, never mutates files, and blocks readiness until the automation stack is healthy so approvals only happen after the canonical gates pass.
- CI artifacts (`observability/audit/AUDIT.REPORT.md`, `observability/audit/AUDIT.METADATA.json`, `scripts/logs/audit-runs.md`, `scripts/logs/process-integrity.md`, `scripts/logs/error-incidents.md`, `git/branches.md`, `TREE.md`, `git/merge-reports/*.md`) must be uploaded so downstream reviewers (including ChatGPT) can inspect deterministic evidence without oral context. Any drift between CI and local artifacts enforces the CI Verification Gate Law plus the Git Automation Exclusivity, Approval-Gated Merge Automation, Mandatory Patch Version, and Process Integrity laws before execution resumes.

## CI-only Force Alignment
- Force-with-lease operations are exclusive to the privileged CI workflow `.github/workflows/ci-force-align.yml`. Humans approve the action via `git/approvals/approved/*-master-force-align.approved`, CI validates the approved SHAs, performs `git push --force-with-lease origin master`, and regenerates `git/branches.md`, the merge report, and the audit artifacts before uploading them so downstream reviewers can verify the remote state without oral context.
- Local automation never executes the force push; the CI Force Executor worker enforces this boundary, blocks TODO closure until the workflow run completes, and logs any CI/local drift through `scripts/logs/error-incidents.md` so the Privileged CI Execution Law stays in force.
