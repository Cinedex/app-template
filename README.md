...
## Remote Canonical Alignment (CI-only)
- Force-with-lease alignment is handled exclusively by the privileged workflow `.github/workflows/ci-force-align.yml`. The workflow runs on `workflow_dispatch`, validates the approval artifact `git/approvals/approved/*-master-force-align.approved`, confirms the approved SHAs match the current local HEAD and `origin/master`, executes `git push --force-with-lease origin master`, and regenerates the branch log, merge report, and audit artifacts before uploading them for review. No local automation runs the force push—CI owns it end-to-end so the Privileged CI Execution Law and CI Force Executor worker can attest to the alignment.
